...
#'
#'
detect.JP.state <- function(dat = list(), window = 14){
  cutoff = 1
  d = dat[, -(1:cutoff)]
  n.day = ncol(d)
  n = nrow(d)
  ...
  trun.scale = quantile(as.matrix(dailynew), probs = 0.75) + 1.5 *
    (quantile(as.matrix(dailynew), probs = 0.75) -
       quantile(as.matrix(dailynew), probs = 0.25))
  ...
  for(i in 1:n){
    ...
    if(nrow(indi) > 0 & nrow(indi) < 4){
      plot(dates, as.numeric(dailynew[i,]), type = "l", main = dat[i, 1], ylab = "Daily Increases")
  ...
  for(i in 1:nrow(ind.all)){
    indi = ind.all[i,]
    print(paste(dat[indi[1], 1], "jump on ", dates[indi[2]], "dailynew being", dailynew[indi[1], indi[2]]))
  }
  }
  n.JP = nrow(ind.all)
  cat("# of jumps =", n.JP, "\n")
