...
#'
#' This function detects the abnormal data point and/or data period of county level covid-19 data (or similar type of data including the count time series and epidemic data).
#'
detect.CP.county <- function(dat = list(), state.show = NULL, county.show = NULL, plot.show = TRUE, cp.sd = 2, slope.r = 5, manual.decision = FALSE){
  ...
  for(i in 1:nrow(dat.sub)){
    ...
    if(sum(!is.na(y)) == 0| sum(y > 0) < 4 | length(unique(unlist(y)) )==2){
      ...
      if(!is.null(est) & max(slope.inc)> slope.r & min(est[,3]) <cp.sd) {
        ...
        cp = ind[cp.x.pos]
        print(paste0("An abnormal point detected at ", counties[i], "," , states[i], " on ", dates[cp] ))
        if(plot.show == TRUE){
          plot(fit_segmented$fitted.values, type = "l", main = paste(counties[i], "," , states[i]), ylab = "Cumulative Counts", xlab = "Date", axes = FALSE)
          points(df)
        ...
        if(manual.decision == TRUE){
          ...
          dates.CP [i, cp:(cp+n.repair)]  = 1
            ...
            plot(fit_segmented$fitted.values, type = "l", main = paste(counties[i], ",", states[i], "CP repaired"), ylab = "Cumulative Counts", xlab = "Date", axes = FALSE)
            points(df)
