...
#'
#' This function repairs the county level covid-19 data (or similar type of data including the count time series and epidemic data) using autoregressive moving average model of count time series.
#'
repair.ARMA.county <- function(dat.I = list(), dat.D = NULL, h = 7){
  dat.rep.I = dat.I
  dat.rep.D = dat.D
  ...
  if(!is.null(dat.I)){
    ...
    for(j in 1:n.loc){
      ...
      for(i in ind.I){
        ...
        mfit.nb = tsglm(diff, model = list(past_obs = h, past_mean = h), distr = "nbinom")
        value.nb = predict(mfit.nb, n.ahead = 1)$median
        mfit.pois = tsglm(diff, model = list(past_obs = h, past_mean = h), distr = "poisson")
